...
public class RedisUtil {
    ...
    private static ResourceBundle resourceBundle = ResourceBundle.getBundle("redis");
    // Redis服务器IP
    private static String IP = resourceBundle.getString("master.redis.ip");
    // Redis的端口号
    private static int port = Integer.parseInt(resourceBundle.getString("master.redis.port")) ;
    // 访问密码
    private static String password = resourceBundle.getString("master.redis.password");
    // 可用连接实例的最大数目，默认值为8；
    // 如果赋值为-1，则表示不限制；如果pool已经分配了maxActive个jedis实例，则此时pool的状态为exhausted(耗尽)。
    private static int max_active = Integer.parseInt( resourceBundle.getString("master.redis.max_active"));
    // 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例，默认值也是8。
    private static int max_idle = Integer.parseInt( resourceBundle.getString("master.redis.max_idle"));
    // 等待可用连接的最大时间，单位毫秒，默认值为-1，表示永不超时。如果超过等待时间，则直接抛出JedisConnectionException；
    private static int max_wait =  Integer.parseInt( resourceBundle.getString("master.redis.max_wait"));
    // 超时时间
    private static int timeout =  Integer.parseInt( resourceBundle.getString("master.redis.timeout"));
